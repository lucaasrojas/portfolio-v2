...
const statusStyles: Record<AppStatus, { label: string; className: string; dot: string }> = {
  live: {
    label: 'Live',
  ...
  idea: {
    label: 'Idea',
    className: 'bg-[var(--bg-surface)] text-[var(--text-muted)] border-[var(--border)]',
        ...
        {apps.length === 0 ? (
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {apps.map((app) => {
              ...
              const CardWrapper = app.internal && app.href
                ? ({ children }: { children: React.ReactNode }) => (
                    <Link href={app.href!} className="block">{children}</Link>
                  )
