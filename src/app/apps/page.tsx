...
const statusStyles: Record<AppStatus, { label: string; dot: string; className: string }> = {
  live:  { label: 'Live',  dot: 'bg-emerald-500 animate-pulse', className: 'text-emerald-700 bg-emerald-50 border-emerald-200' },
  wip:   { label: 'WIP',   dot: 'bg-amber-400',                 className: 'text-amber-700 bg-amber-50 border-amber-200' },
  idea:  { label: 'Idea',  dot: 'bg-[var(--text-muted)]',       className: 'text-[var(--text-muted)] bg-[var(--bg-surface)] border-[var(--border)]' },
}
      ...
      {internalApps.length === 0 ? (
        <div className="border-2 border-dashed border-[var(--border-dark)] rounded-2xl p-16 text-center">
          ...
          <p className="font-mono text-[var(--text-muted)] text-sm">
            ...
            en <span className="text-[var(--text-secondary)]">src/data/portfolio.ts</span>
          </p>
        </div>
      ) : (
            ...
            return (
              ...
              >
                <div className="flex items-start justify-between mb-4">
                  <span className={`inline-flex items-center gap-1.5 font-mono text-[11px] font-semibold px-2.5 py-1 rounded-full border ${s.className}`}>
                ...
                {app.icon && <span className="text-3xl mb-3 block">{app.icon}</span>}
                ...
                <div className="flex flex-wrap gap-1.5 mb-4">
                  {app.tags.map((tag) => (
                    <span key={tag} className="font-mono text-[10px] bg-[var(--bg-surface)] border border-[var(--border)] text-[var(--text-muted)] px-2 py-0.5 rounded-md">
                      {tag}
                    </span>
                  ))}
                </div>
